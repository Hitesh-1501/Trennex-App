        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    ...
                    if (target == null) {
                        // Only pop if collections were actually loaded and the ID is now invalid
                        if (collections.isNotEmpty() && !didPopAfterDelete && findNavController().currentDestination?.id == R.id.collectionItemsFragment) {
